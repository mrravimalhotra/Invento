...
## Data
`equipment(equipment_code, name, room_no, section, legacy_asset_id,
...
of the 304 seeded rows carry a `legacy_asset_id` (the existing
printed/engraved tag, e.g. `ANHC/QC/07/205(B1)`); the remaining 30 are
bare-count entries with no individual tag (e.g. "Silica crucible — 2").
